...
public interface BookRepository extends JpaRepository<Book, Long> {
    ...
    @Query("SELECT b FROM Book b WHERE (:title IS NULL OR b.title LIKE CONCAT('%', :title, '%')) AND (:author IS NULL OR b.author LIKE CONCAT('%', :author, '%')) AND (:isbn IS NULL OR b.isbn = :isbn)")
